...
async function onSubmit() {
  ...
  try {
    ...
    if (err.response?.data?.error_code in BotErrorCode) {
      Message.error({
        content: '重新下发失败：' + BotErrorCode[err.response?.data?.error_code],
        duration: 6000,
      });
    } else {
    Message.error({
      content: '重新下发失败：' + err.response?.data?.description,
        duration: 6000,
      });
    }
  }
...
<template>
  <Spin class='form-wrapper' :loading='status === "loading"' tip='正在重新下发'>
    <Form
      class='form'
      :model='input'
    ...
    >
      ...
      <FormItem
        label='勋章 ID'
        field='credit.id'
        tooltip='需为已下发的勋章 ID，重新下发/修改时将覆盖该勋章'
